class PrayOptionDisplay {
    ...
    render(){
        this.container.innerHTML = `<div style="font-size: large;"> ✔️ 함온성 설정  <span style="font-size: 0.8rem;"> (네모박스를 클릭하면 활성화됩니다) </span> </div>
            <div>
                <input type="checkbox" id="prayForUserCheckBox" name="prayForUserCheckBox" ${this.isPrayForUserVisible ? 'checked' : ''}> 
                <label for="prayForUserCheckBox">앞사람을 위한 기도하기</label>
            </div>
            <div>
                <input type="checkbox" id="prayCheckBox" name="prayCheckBox" ${this.isPrayVisible ? 'checked' : ''}> 
